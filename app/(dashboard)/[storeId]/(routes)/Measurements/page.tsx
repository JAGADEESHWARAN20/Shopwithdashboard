...
import { getMeasurements } from "@/lib/data/measurement";
...
const MeasurementsPage = async ({ params }: { params: Promise<{ storeId: string }> }) => {
  ...
  const data: MeasurementRow[] = measurements.map((m) => ({
    id: m.id,
    name: m.name,
    fieldCount: Array.isArray((m.fields as { measurements?: unknown[] } | null)?.measurements)
      ? ((m.fields as { measurements: unknown[] }).measurements.length)
      : 0,
